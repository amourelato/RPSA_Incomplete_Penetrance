"""
Identifies regions that are not identical by state (IBS) on chr3 carrying the WT copy for PRSA for siblings in ICA-AS
...
def main():
    ...
    vcf_unphased = vcf_unphased.drop(['ID', 'QUAL', 'FILTER', 'INFO', 'FORMAT', 'JM9146','JM9146','JM9375'], axis=1) 
    ...
    df_homo_parent = df[df['JM9402'].str.contains('0/0|1/1')]
    
    #Perform the mendelian phasing
    df[['child1_WT', 'child2_WT']] = df.apply(
        mendelian_phase, axis=1,
        parent_col='JM9402', child1_col='JM9401', child2_col='JM9403'
    )
