...
def BlockBootstrap(Fam_Dict, Individual_DF, iterations):
    ...
    while len(Odds_Ratio) < iterations:
        ...
        #define a new table based on the bootstrap replicate, grouping G1/G2/G3 together
        #the table will be a 2x2 Rows are Group (G0 or G1+G2+G3), Columns are Spleen Phenotype (ICA or Spleen)
        table = np.array([[b, a],[(d+f+h), (c+e+g)]])
